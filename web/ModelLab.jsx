...
import './models.css';
import evaluation from './assets/evaluation.json' with {type:'json'};
...
export default function ModelLab({assetBase='/assets/computer-vision/'}) {
 ...
 const detection=model.kind==='detect'||model.kind==='pose';
 const checked=evaluation.models[modelId];
 ...
 useEffect(()=>{
  ...
  if(!example){setSource(null);setStatus('Upload a tiger photo to try this prototype.');return;}
  setSource({title:classLabel(example.label)+' example',src:base+'model-examples/'+example.file,id:example.file});
 },[modelId,base]);
 useEffect(()=>{
  if(!source){setPixels(null);setResult(null);if(canvas.current)canvas.current.getContext('2d').clearRect(0,0,canvas.current.width,canvas.current.height);return;}
 ...
 useEffect(()=>{
  ...
  for(const box of result?.boxes||[]){
   ...
   const text=`${classLabel(box.label)} · ${Math.round(box.score*100)}%`;ctx.font='bold 14px monospace';
   ...
   if(box.keypoints){
    ctx.strokeStyle='#f1c58b';ctx.lineWidth=2;
    for(const [a,b] of model.skeleton||[]){const first=box.keypoints[a],second=box.keypoints[b];if(first?.inFrame&&second?.inFrame){ctx.beginPath();ctx.moveTo(first.x,first.y);ctx.lineTo(second.x,second.y);ctx.stroke();}}
    ctx.fillStyle='#f1c58b';for(const point of box.keypoints)if(point.inFrame){ctx.beginPath();ctx.arc(point.x,point.y,3,0,Math.PI*2);ctx.fill();}
   }
  }
 },[pixels,result,cutoff,model]);
 ...
 function saveImage(){canvas.current.toBlob(blob=>{if(!blob)return;const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=modelId==='tiger'?'tiger-keypoints.png':'alpaca-detections.png';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);});}
 return <section className="vision-lab vision-model-lab" aria-label="Trained image models" data-model={modelId} data-state={busy?'running':result?'complete':'idle'}>
  ...
  <div className="vision-model-intro"><h2>{model.description}</h2><p>{model.hint}</p>{modelId==='tiger'&&examples.length>0&&<p className="vision-model-note">Examples are validation frames used to select the checkpoint, not new test images. <a href="https://youtu.be/Gc6K5eKrTNQ">Source video ↗</a></p>}</div>
  ...
  <div className="vision-model-workspace">
   <figure className="vision-model-image"><figcaption>{source?.title||(modelId==='tiger'?'Upload a tiger photo':'Opening example…')}</figcaption><canvas ref={canvas} width="768" height="512" aria-label={detection?'Image with detected animals':'Image to classify'}/></figure>
   <aside className="vision-model-results" aria-label="Prediction results">
    ...
    {result?.kind==='classify'&&<><h3>{classLabel(result.label)}</h3><ol className="vision-scores">{result.scores.map(item=><li key={item.label}><div><span>{classLabel(item.label)}</span><strong>{item.score<.001?'<0.1':(item.score*100).toFixed(1)}%</strong></div><meter min="0" max="1" value={item.score} aria-label={`${classLabel(item.label)} model score`}/></li>)}</ol><p className="vision-model-note">Scores compare the model’s known classes; they are not a guarantee of correctness.</p></>}
    {result?.kind==='svm'&&<><h3>{classLabel(result.label)}</h3><p className="vision-margin">Decision margin <strong>{result.margin.toFixed(3)}</strong></p><p className="vision-model-note">Positive means occupied; negative means empty. This SVM does not output calibrated probabilities.</p></>}
    {detection&&<fieldset className="vision-cutoff"><legend>Detection cutoff</legend><div role="group" aria-label="Detection cutoff">{[[.6,'Strict'],[.35,'Balanced'],[.15,'Explore']].map(([value,label])=><button type="button" key={value} aria-pressed={cutoff===value} onClick={()=>setCutoff(value)}>{label}</button>)}</div>{result&&<p>{boxes.length} {modelId==='tiger'?'tiger':'alpaca'}{boxes.length===1?'':'s'} above {Math.round(cutoff*100)}%{!boxes.length?' · Try a clearer image or a lower cutoff.':''}</p>}</fieldset>}
    {modelId==='tiger'&&<p className="vision-model-note">Dots show 12 predicted landmarks. Occluded points may be guesses; this model has no landmark visibility scores.</p>}
    {result&&<div className="vision-model-exports"><button type="button" onClick={save}>Save results</button>{detection&&<button type="button" onClick={saveImage}>Save annotated PNG</button>}</div>}
    <p className="vision-model-note">{model.revision?'Retrained':'Archived'} {model.kind==='svm'?'SVM':'YOLOv8'} model · {(model.bytes/1048576).toFixed(1)} MB{model.kind!=='svm'?' + shared inference runtime':''}. Downloads only when you run it.</p>
   </aside>
  </div>
  {checked&&<div className="vision-evaluation"><h3>How this model was checked</h3><p><strong>{(checked.value*100).toFixed(1)}% {checked.metric}</strong> on {checked.samples.toLocaleString()} held-out {modelId==='tiger'?'frames':'images'}. {checked.split}.{checked.map50_95!=null&&` mAP50–95: ${(checked.map50_95*100).toFixed(1)}%.`}</p><p>{checked.limit}</p><a href="https://github.com/Zachshotamartin/ComputerVisionLab/blob/main/docs/EVALUATION.md">Read the evaluation and failure cases ↗</a></div>}
  <p className="vision-privacy">Images stay on your device. Only the selected model downloads; inference runs in a browser worker. Evaluation results apply to the documented datasets and splits.</p>
 </section>;
}
