  ...
  return <section className="vision-lab" aria-label="Interactive computer vision lab" data-ready={Boolean(metrics)}>
    ...
    <div className="vision-workspace">
      <aside className="vision-controls">
        ...
        {tracking ? <><ColorPicker value={color} onChange={setColor} /><label>Hue tolerance <output>{tolerance}°</output><input type="range" min="2" max="90" value={tolerance} onChange={event => setTolerance(Number(event.target.value))} /></label></> : <label>{mode === 'redact' ? 'Pixel size' : mode === 'blur' ? 'Blur strength' : 'Threshold'} <output>{amount}</output><input aria-label={mode === 'redact' ? 'Pixel size' : mode === 'blur' ? 'Blur strength' : 'Threshold'} type="range" min="1" max="255" value={amount} onChange={event => setAmount(Number(event.target.value))} /></label>}
