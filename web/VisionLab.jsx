...
export default function VisionLab({ assetBase = '/assets/computer-vision/' }) {
  ...
  useEffect(() => {
    const options = { mode, amount, color, tolerance, regions, minArea: 50 };
    currentOptions.current = options;
    submit(lastImage.current || pixels, options);
  }, [mode, amount, color, tolerance, regions, pixels, submit]);
  ...
  function point(event) {
    const rect = after.current.getBoundingClientRect();
    return { x: (event.clientX - rect.left) * after.current.width / rect.width, y: (event.clientY - rect.top) * after.current.height / rect.height };
  }
  ...
  return <section className="vision-lab" aria-label="Interactive computer vision lab" data-ready={Boolean(metrics)}>
    ...
    <div className="vision-workspace">
      <aside className="vision-controls">
        <label className="vision-select">Operation<select value={mode} onChange={event => { setMode(event.target.value); setError(''); }} aria-label="Vision operation">{visionModes.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>
        <p>{selected.description}</p>
        {/* Host color pickers can update React's tracked value before emitting input. */}
        {tracking ? <><label>Target color<input type="color" value={color} onInput={event => setColor(event.currentTarget.value)} onChange={event => setColor(event.currentTarget.value)} /></label><label>Hue tolerance <output>{tolerance}°</output><input type="range" min="2" max="90" value={tolerance} onChange={event => setTolerance(Number(event.target.value))} /></label></> : <label>{mode === 'redact' ? 'Pixel size' : mode === 'blur' ? 'Blur strength' : 'Threshold'} <output>{amount}</output><input aria-label={mode === 'redact' ? 'Pixel size' : mode === 'blur' ? 'Blur strength' : 'Threshold'} type="range" min="1" max="255" value={amount} onChange={event => setAmount(Number(event.target.value))} /></label>}
        ...
        <div className="vision-readout"><span>{camera ? 'Live camera · up to 10 fps' : source.title}</span><strong>{metrics ? `${metrics.width} × ${metrics.height}` : 'Opening image…'}</strong>{metrics && <span>{tracking || mode === 'redact' ? `${metrics.regions} region${metrics.regions === 1 ? '' : 's'} · ` : ''}{metrics.milliseconds.toFixed(1)} ms processing</span>}</div>
        <button className="vision-export" type="button" onClick={exportImage} disabled={!metrics || loading}>Export result</button>
      </aside>
      <div className="vision-images" aria-busy={loading}>
        <figure><figcaption><span>01</span> Source</figcaption><canvas ref={before} aria-label="Source image" width="768" height="512" /></figure>
        <figure><figcaption><span>02</span> {selected.label}{mode === 'redact' && <small>Drag to select</small>}</figcaption><canvas ref={after} aria-label="Processed image; in redaction mode drag to select a region" className={mode === 'redact' ? 'vision-redaction-canvas' : ''} width="768" height="512" onPointerDown={event => { if (mode === 'redact') { pointer.current = point(event); event.currentTarget.setPointerCapture(event.pointerId); } }} onPointerUp={finishRegion} onPointerCancel={() => { pointer.current = null; }} /></figure>
      </div>
    </div>
