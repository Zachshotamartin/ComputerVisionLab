...
def parser():
    cli = argparse.ArgumentParser(description='Computer vision experiments: portable inputs, reproducible runs, explicit validation.')
    commands = cli.add_subparsers(dest='command', required=True)
    image = commands.add_parser('image', help='Process a still image')
    image.add_argument('source', type=Path)
    image.add_argument('--mode', choices=['original', 'gray', 'edges', 'threshold', 'adaptive', 'blur', 'contours'], default='edges')
    ...
    for name in ('track', 'redact'):
        camera = commands.add_parser(name, help='Process an image, video, or camera; Q exits')
    ...
    train = commands.add_parser('train', help='Validate before starting a YOLO training run')
    ...
    train.add_argument('--dry-run', action='store_true', help='Check data without importing the model or training')
    for name in ('predict', 'evaluate'):
        model = commands.add_parser(name)
        model.add_argument('--model', type=Path, required=True)
        model.add_argument('--source' if name == 'predict' else '--data', type=Path, required=True)
    ...
    parking = commands.add_parser('parking', help='Train or apply the parking-space SVM')
    ...
    parking.add_argument('--limit', type=int, default=0, help='Optional images per class; 0 uses all')
    return cli
...
def run(args):
    if args.command == 'train':
        ...
        print(json.dumps({'task': args.task, 'dataset': report, 'seed': args.seed}, indent=2))
        if args.dry_run:
            return
        ...
        if isinstance(data, dict):
            ...
            data = str(config)
        pretrained = {'classify': 'yolov8n-cls.pt', 'detect': 'yolov8n.pt', 'pose': 'yolov8n-pose.pt'}
        model = load_model(args.model or pretrained[args.task])
        if model.task != args.task:
            raise ValueError(f'The checkpoint task is {model.task}, but --task is {args.task}')
        with local_checkpoints():
            model.train(data=data, epochs=args.epochs, imgsz=args.imgsz, batch=args.batch,
                        seed=args.seed, deterministic=True, workers=0, device=args.device,
                        project=str(args.output.resolve()), name='run', exist_ok=False)
        return
    if args.command in ('predict', 'evaluate'):
        ...
        if args.command == 'evaluate':
            if model.task == 'classify':
                ...
                raise ValueError('Use the task-specific YOLO validation command with a separately annotated test split for detection/pose.')
            metrics = model.val(data=data, split='test', imgsz=args.imgsz, device=args.device, workers=0)
            (args.output / 'metrics.json').write_text(json.dumps(metrics.results_dict, indent=2))
            return
        if not args.source.exists():
            raise ValueError(f'Input does not exist: {args.source}')
        destination = args.output / 'predictions.jsonl'
        if destination.exists():
            raise ValueError(f'Prediction output already exists: {destination}. Choose a new --output directory.')
        written = 0
        for result in model.predict(source=str(args.source.resolve()), imgsz=args.imgsz, conf=args.confidence,
                                    device=args.device, stream=True, verbose=False):
            row = {'source': Path(result.path).name}
            if result.probs is not None:
                scores = result.probs.data.cpu().tolist()
                row['predictions'] = sorted([{'label': result.names[i], 'score': float(p)} for i, p in enumerate(scores)], key=lambda p: p['score'], reverse=True)
            if result.boxes is not None:
                row['boxes'] = [{'xyxy': box[:4], 'score': box[4], 'label': result.names[int(box[5])]} for box in result.boxes.data.cpu().tolist()]
            ...
            if written == 0 and args.source.is_file() and args.source.suffix.lower() in ('.png', '.jpg', '.jpeg', '.webp'):
    ...
    if args.command == 'parking':
        from .parking import train, predict
        if args.source:
            print(json.dumps(predict(args.model, args.source), indent=2))
        elif args.data:
            print(json.dumps(train(args.data, args.model, args.seed, args.limit), indent=2))
        else:
    ...
    if args.command == 'image':
        ...
        return
    detector = imaging.face_detector() if args.command == 'redact' else None
    ...
    if Path(args.source).suffix.lower() in ('.png', '.jpg', '.jpeg', '.webp'):
        imaging.write_image(args.output or Path(f'output/{args.command}.png'), transform(imaging.read_image(args.source)))
    else:
        if args.headless and not args.output:
            raise ValueError('Headless video processing needs --output')
        print(f'Processed {imaging.video_loop(args.source, transform, args.output, not args.headless)} frames')
